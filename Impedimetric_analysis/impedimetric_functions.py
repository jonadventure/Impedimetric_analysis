...
def Analysis_EIS(values_row_start_get, x_column_get,y_column_get,x_start_get, x_end_get,
    ...
    cir_pt2_index = int(cir_pt2_index) #to circle fit, nr. index away to left of max of circle!!
    ...
    try:
        ...
        try:
            for j in range(0,len(filename)):
                ...
                circle_max_index = sorted_data.iloc[:circle_end_index,1].idxmax() #sort data points (so all points are in right order)
                xdata = sorted_data.iloc[cir_pt1_index:circle_max_index,0] #only include data from start to max of semicircle
                ydata = sorted_data.iloc[cir_pt1_index:circle_max_index,1] #only include data from start to max of semicircle
                popt, pcov = curve_fit(func, np.array(xdata), np.array(ydata**2)) #solve non linear system
